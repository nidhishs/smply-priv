...
from ..transforms import (addDPG, affine_transform, batch_rodrigues_numpy,
                          ...
                          flip_twist, flip_xyz_joints_3d, get_affine_transform,
                          im_to_torch, rot_aa, rotate_xyz_jts)
...
class SimpleTransform3DSMPLCam(object):
    ...
    def __init__(
        ...
        self.depth_factor2meter = (
            self.bbox_3d_shape[2]
            if self.bbox_3d_shape[2] < 500
            else self.bbox_3d_shape[2] * 1e-3
        )
    ...
    def _integral_uvd_target_generator(
        self, joints_3d, num_joints, patch_height, patch_width
    ):
    ...
    def __call__(self, src, label):
        if self.two_d:
            ...
            joints_xyz = (
                joint_cam - joint_cam[[self.root_idx]].copy()
            )  # the root index of mpii_3d is 4 !!!
            ...
            intrinsic_param = (
                get_intrinsic_metrix(label["f"], label["c"], inv=True).astype(
                    np.float32
                )
                if "f" in label.keys()
            ...
            target_xyz, target_xyz_weight = self._integral_xyz_target_generator(
                joints_xyz, joints_vis, len(joints_vis)
            )

            target_weight *= joints_vis.reshape(-1)
            bbox = _center_scale_to_box(center, scale)

            cam_scale, cam_trans, cam_valid, cam_error, new_uvd = (
                self.calc_cam_scale_trans2(
            ...
            # half body transform
            if self._train and (
                np.sum(joints_vis_17[:, 0]) > self.num_joints_half_body
                and np.random.rand() < self.prob_half_body
            ):
                # self.num_joints = 17
                # c_half_body, s_half_body = self.half_body_transform(
            ...
            if random.random() > 0.75 and self._train:
                ...
                joints_17_uvd = flip_joints_3d(
                    joints_17_uvd, imgwidth, self._joint_pairs_17
                )
                joints_29_uvd = flip_joints_3d(
                    joints_29_uvd, imgwidth, self._joint_pairs_29
                )
                joint_cam_17_xyz = flip_cam_xyz_joints_3d(
                    joint_cam_17_xyz, self._joint_pairs_17
                )
                joints_cam_24_xyz = flip_cam_xyz_joints_3d(
                    joints_cam_24_xyz, self._joint_pairs_24
                )
                theta = flip_thetas(theta, self._joint_pairs_24)
                twist_phi, twist_weight = flip_twist(
                    twist_phi, twist_weight, self._joint_pairs_24
                )
                center[0] = imgwidth - center[0] - 1
            ...
            for i in range(17):
                if joints_17_uvd[i, 0, 1] > 0.0:
                    joints_17_uvd[i, 0:2, 0] = affine_transform(
                        joints_17_uvd[i, 0:2, 0], trans
                    )

            for i in range(29):
                if joints_29_uvd[i, 0, 1] > 0.0:
                    joints_29_uvd[i, 0:2, 0] = affine_transform(
            ...
            # generate training targets
            target_uvd_29, target_weight_29 = self._integral_uvd_target_generator(
                joints_29_uvd, 29, inp_h, inp_w
            )
            target_xyz_17, target_weight_17 = self._integral_xyz_target_generator(
                joints_17_xyz, joints_vis_17, 17
            )
            target_xyz_24, target_weight_24 = self._integral_xyz_target_generator(
                joints_24_xyz, joints_vis_29[:24, :], 24
            )
    ...
    def calc_cam_scale_trans2(self, xyz_29, uvd_29, uvd_weight):
        ...
        backed_projected_xyz[:, 2] = (
            backed_projected_xyz[:, 2] * self.depth_factor2meter
        )
    ...
    def back_projection(self, uvd, pred_camera, focal_length=5000.0):
        ...
        pred_xyz[:, :2] = (uvd[:, :2] * 256 / focal_length) * (
            pred_xyz[:, 2:] * self.depth_factor2meter + camDepth
        ) - camTrans

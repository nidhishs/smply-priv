...
from ..transforms import (addDPG, affine_transform, batch_rodrigues_numpy,
                          flip_cam_xyz_joints_3d, flip_joints_3d, flip_thetas,
                          flip_twist, get_affine_transform, im_to_torch,
                          rot_aa, rotate_xyz_jts, rotmat_to_quat_numpy)
...
class SimpleTransform3DSMPLX(object):
    ...
    def __init__(
        ...
        self.leaf_joint_pairs = [(55, 56), (58, 59)] + [
            (60 + i, 65 + i) for i in range(5)
        ]

        self.joint_pairs_71 = (
            self.body_joint_pairs + self.hand_joint_pairs + self.leaf_joint_pairs
        )
        ...
        if self.update_beta:
            ...
            for gender, layer in smplx_layers_dict.items():
                shape_dir = torch.cat(
                    [layer.shapedirs, layer.expr_dirs], dim=-1
                )  # num_v x 3 x 21
                ...
                self.normed_shapedirs_dict[gender] = normed_shape_disps.reshape(
                    num_v, 3, 10
                )  # num_v x 3 x 10
    ...
    def __call__(self, src, label):
        ...
        if self._train and (
            (np.sum(joint_vis[:24, 0]) > self.num_joints_half_body)
        ...
        if self._train and self._occlusion and False:
            ...
            while True:
                ...
                area_max = 0.3
                synth_area = (
                    (random.random() * (area_max - area_min) + area_min)
                    * (xmax - xmin)
                    * (ymax - ymin)
                )
                ...
                if (
                    synth_xmin >= 0
                    and synth_ymin >= 0
                    and synth_xmin + synth_w < imgwidth
                    and synth_ymin + synth_h < imght
                ):
                    ...
                    synth_h = int(synth_h)
                    src[
                        synth_ymin : synth_ymin + synth_h,
                        synth_xmin : synth_xmin + synth_w,
                        :,
                    ] = (
        ...
        if random.random() > 0.5 and self._train:
            ...
            twist_phi, twist_weight = flip_twist(
                twist_phi, twist_weight, self.joint_pairs_55
            )
        ...
        trans_inv = get_affine_transform(
            center, scale, r, [inp_w, inp_h], inv=True
        ).astype(np.float32)
        intrinsic_param = (
            get_intrinsic_metrix(label["f"], label["c"], inv=True).astype(np.float32)
            if "f" in label.keys()
            else np.zeros((3, 3)).astype(np.float32)
        )
        joint_root = (
            label["root_cam"].astype(np.float32)
            if "root_cam" in label.keys()
            else np.zeros((3)).astype(np.float32)
        )
        depth_factor = (
            np.array([self.bbox_3d_shape[2]]).astype(np.float32)
            if self.bbox_3d_shape
            else np.zeros((1)).astype(np.float32)
        )
        ...
        if self.update_beta:
            beta_new, _ = self.correct_gendered_beta(
                beta_full_torch, expression_torch, [gender]
            )
        ...
        target_uvd, target_weight_uvd = self._integral_uvd_target_generator(
            joints_uvd, inp_h, inp_w
        )
        ...
        if self.focal_length > 0:
            ...
            xyz_uvd_weight = (
                target_weight_uvd.reshape(-1, 3)[:55]
                * target_weight_xyz.reshape(-1, 3)[:55]
            )
        ...
        output = {
            ...
            "target_weight_uvd": torch.from_numpy(
                target_weight_uvd.reshape(-1)
            ).float(),
    ...
    def correct_gendered_beta(self, beta_gender, expression_gender, gender_list):
        ...
        v_shaped_gender = self.v_template_dict[gender] + torch.einsum(
            "bl,mkl->bmk", [shape_components, self.shapedirs_dict[gender]]
        )

        # v_shaped_gender = self.v_template_dict[gender] + torch.einsum('bl,mkl->bmk', [beta_gender, self.shapedirs_dict[gender][:, :, :11]])

        expression_v_neutral = torch.einsum(
            "bl,mkl->bmk",
            [expression_gender, self.shapedirs_dict["neutral"][:, :, 11:]],
        )
        kid_v_neutral = torch.einsum(
            "bl,mkl->bmk", [beta_last, self.kid_shapedir_dict["neutral"]]
        )

        v_shaped_neutral_residual = (
            v_shaped_gender
            - self.v_template_dict["neutral"]
            - expression_v_neutral
            - kid_v_neutral
        )
        ...
        betas_regressed = (
            torch.einsum(
                "bmk,mkl->bl",
                [v_shaped_neutral_residual, self.normed_shapedirs_dict["neutral"]],
            )
            / self.shapedirs_norm_dict["neutral"]
        )

        betas_regressed_full = torch.cat([betas_regressed, beta_last], dim=-1)

        shape_components_new = torch.cat(
            [betas_regressed_full, expression_gender], dim=-1
        )
        original_neutral_v = (
            torch.einsum(
                "bl,mkl->bmk", [shape_components, self.shapedirs_dict["neutral"]]
            )
            + self.v_template_dict["neutral"]
        )
        generated_neutral_v = (
            torch.einsum(
                "bl,mkl->bmk", [shape_components_new, self.shapedirs_dict["neutral"]]
            )
            + self.v_template_dict["neutral"]
        )
...
def back_projection(uvd, pred_camera, focal_length=5000.0):
...
def back_projection_matrix(uv, xyz, pred_camera, img_center):
    ...
    cx, cy = img_center
    intrinsic_inv = np.array([[1 / f, 0, -cx / f], [0, 1 / f, -cy / f], [0, 0, 1]])

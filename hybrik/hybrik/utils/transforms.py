...
def flip_heatmap_dz(
    heatmap, joint_pairs, bone_pairs, num_joints, num_bones, shift=False
):
    assert heatmap.dim() == 3 or heatmap.dim() == 4
    out = flip(heatmap)
    ...
    for pair in bone_pairs:
        ...
        dv_idx = torch.Tensor(
            (dim0 + num_joints + num_bones, dim1 + num_joints + num_bones)
        ).long()
        dv_inv_idx = torch.Tensor(
            (dim1 + num_joints + num_bones, dim0 + num_joints + num_bones)
        ).long()
...
def flip_twist(twist_phi, twist_weight, twist_pairs):
    ...
    for pair in twist_pairs:
        ...
        twist_flip[idx0, :], twist_flip[idx1, :] = (
            twist_flip[idx1, :],
            twist_flip[idx0, :].copy(),
        )

        weight_flip[idx0, :], weight_flip[idx1, :] = (
            weight_flip[idx1, :],
            weight_flip[idx0, :].copy(),
        )
...
def norm_heatmap(norm_name, heatmap):
    ...
    ), "Heatmap to be normalized must be torch.Tensor!"
    shape = heatmap.shape
...
recover_z = {"pw3d": pw3d_recover_z, "h36m": h36m_recover_z, "hp3d": hp3d_recover_z}


def heatmap_to_coord(
    pred_jts, pred_scores, hm_shape, bbox, output_3d=False, mean_bbox_scale=None
):
...
def rot_theta(theta, xyz_coord, rot, smpl_parents):
    ...
    theta_0 = theta[
        :1
    ]  # the first theta is global orientation, and should be processed separately
    theta_left = theta[1:]

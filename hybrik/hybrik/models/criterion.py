...
def weighted_laplace_loss(input, sigma, target, weights, size_average):
    input = input
    target = target
    out = torch.log(sigma / amp) + torch.abs(input - target) / (
        math.sqrt(2) * sigma + 1e-5
    )
    out = out * weights
    if size_average and weights.sum() > 0:
        return out.sum() / weights.sum()
...
class L1LossDimSMPL(nn.Module):
    ...
    def forward(self, output, labels):
        ...
        # SMPL params
        loss_beta = self.criterion_smpl(
            output.pred_shape * smpl_weight, labels["target_beta"] * smpl_weight
        )
        loss_theta = self.criterion_smpl(
            output.pred_theta_mats * smpl_weight * labels["target_theta_weight"],
            labels["target_theta"] * smpl_weight * labels["target_theta_weight"],
        )
        loss_twist = self.criterion_smpl(
            output.pred_phi * labels["target_twist_weight"],
            labels["target_twist"] * labels["target_twist_weight"],
        )
        ...
        loss_uvd = weighted_l1_loss(
            output.pred_uvd_jts, target_uvd, target_uvd_weight, self.size_average
        )
...
class L1LossDimSMPLCam(nn.Module):
    ...
    def forward(self, output, labels, epoch_num=0):
        ...
        loss_xyz = weighted_l1_loss(
            pred_xyz, target_xyz, target_xyz_weight, self.size_average
        )
        ...
        loss_uvd = weighted_l1_loss(
            ...
            target_uvd_weight.reshape(batch_size, -1),
            self.size_average,
        )

...
def norm_heatmap(norm_type, heatmap, tau=5, sample_num=1):
    ...
    if norm_type == "softmax":
        ...
        eps = torch.rand(
            *heatmap.shape[:2], sample_num, heatmap.shape[3], device=heatmap.device
        )
        log_eps = torch.log(-torch.log(eps))
        gumbel_heatmap = heatmap - log_eps / tau
        gumbel_heatmap = F.softmax(gumbel_heatmap, 3)
        gumbel_heatmap = gumbel_heatmap.reshape(
            shape[0], shape[1], sample_num, shape[2]
        )
...
class HRNetSMPLXCamKidReg(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        bbox_3d_shape = (
            kwargs["BBOX_3D_SHAPE"] if "BBOX_3D_SHAPE" in kwargs else (2200, 2200, 2200)
        )
    ...
    def update_scale(
        self, pred_uvd, weight, init_scale, pred_shape_full, pred_phi, **kwargs
    ):
        ...
        if "bboxes" in kwargs.keys():
            ...
            pred_xyz[:, :, 2:] = pred_uvd[
                :, :, 2:
            ].clone()  # unit: (self.depth_factor m)
            pred_xy = (
                (pred_uvd[:, :, :2] + bbox_center) * self.input_size / self.focal_length
            ) * (
                pred_xyz[:, :, 2:] * self.depth_factor + cam_depth
            )  # unit: m

            pred_xyz[:, :, :2] = (
                pred_xy / self.depth_factor
    ...
    def forward(self, x, flip_test=False, **kwargs):
        ...
        out_sigma = (
            self.decsigma(x0).sigmoid().reshape(batch_size, (self.num_joints - 1), 1)
        )

        out_coord_mouthtop = self.fc_coord_mouthtop(x0).reshape(batch_size, 1, 3)
        out_sigma_mouthtop = (
            self.decsigma_mouthtop(x0).sigmoid().reshape(batch_size, 1, 1)
        )
        ...
        if flip_test:
            ...
            flip_out_coord = self.fc_coord(flip_x0).reshape(
                batch_size, (self.num_joints - 1), 3
            )
            flip_out_sigma = (
                self.decsigma(flip_x0)
                .sigmoid()
                .reshape(batch_size, (self.num_joints - 1), 1)
            )

            flip_out_coord_mouthtop = self.fc_coord_mouthtop(flip_x0).reshape(
                batch_size, 1, 3
            )
            flip_out_sigma_mouthtop = (
                self.decsigma_mouthtop(flip_x0).sigmoid().reshape(batch_size, 1, 1)
            )
        ...
        if self.hand_relative:
            ...
            out_coord[:, self.left_fingers_ids, :] = (
                left_hand_uvd + out_coord[:, [self.left_wrist_id], :]
            )
            out_coord[:, self.right_fingers_ids, :] = (
                right_hand_uvd + out_coord[:, [self.right_wrist_id], :]
            )
        else:
            left_hand_uvd = (
                out_coord[:, self.left_fingers_ids, :]
                - out_coord[:, [self.left_wrist_id], :]
            )
            right_hand_uvd = (
                out_coord[:, self.right_fingers_ids, :]
                - out_coord[:, [self.right_wrist_id], :]
            )
        ...
        if "bboxes" in kwargs.keys():
            ...
            pred_xyz_jts[:, :, 2:] = pred_uvd_jts[
                :, :, 2:
            ].clone()  # unit: (self.depth_factor m)
            pred_xy_jts_29_meter = (
                (pred_uvd_jts[:, :, :2] + bbox_center)
                * self.input_size
                / self.focal_length
            ) * (
            ...
            pred_xyz_jts[:, :, :2] = (
                pred_xy_jts_29_meter / self.depth_factor
            )  # unit: (self.depth_factor m)
            ...
            pred_xy_jts_29_meter = (
                pred_uvd_jts[:, :, :2] * self.input_size / self.focal_length
            ) * (
                pred_xyz_jts[:, :, 2:] * self.depth_factor + camDepth
            )  # unit: m
        ...
        #  -0.5 ~ 0.5
        pred_xyz_hybrik_struct = self.smplx_layer.get_extended_joints(
            output.joints[:, :55].clone(), pred_vertices
        )
        ...
        if "gt_labels" in kwargs.keys() and kwargs["gt_labels"] is not None:
            ...
            gt_beta = torch.cat(
                [gt_labels["target_beta"], gt_labels["target_beta_kid"]], dim=1
            )
            with torch.no_grad():
        ...
        pred_uv_full = (
            (pred_xyz_full[:, :, :2] / pred_xyz_full[:, :, 2:])
            * self.focal_length
            / self.input_size
        )

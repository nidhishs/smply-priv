...
class Simple3DPoseBaseSMPL(nn.Module):
    ...
    def forward(
        ...
        if flip_item is not None:
            assert flip_output
            pred_uvd_jts_29_orig, pred_phi_orig, pred_leaf_orig, pred_shape_orig = (
                flip_item
            )
        ...
        if flip_output and flip_item is not None:
            pred_uvd_jts_29 = (
                pred_uvd_jts_29 + pred_uvd_jts_29_orig.reshape(batch_size, 29, 3)
            ) / 2
        ...
        #  -0.5 ~ 0.5
        # Rotate back
        pred_xyz_jts_29 = self.uvd_to_cam(
            pred_uvd_jts_29, trans_inv, intrinsic_param, joint_root, depth_factor
        )
        assert torch.sum(torch.isnan(pred_xyz_jts_29)) == 0, (
            "pred_xyz_jts_29",
            pred_xyz_jts_29,
        )

        pred_xyz_jts_29 = pred_xyz_jts_29 - pred_xyz_jts_29[
            :, self.root_idx_smpl, :
        ].unsqueeze(1)

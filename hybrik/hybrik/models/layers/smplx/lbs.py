...
def batch_inverse_kinematics_transform_naive(
    ...
    representative_children = {9: 12, 15: 70, 20: 25, 21: 40}
    # leaf nodes rot_mats
    if leaf_thetas is not None:
        ...
        if children[i] == -1:
            ...
            if i == 15 and False:
                ...
                if train:
                    ...
                    middle_final_loc = (
                        rel_pose_skeleton[:, c1] + rel_pose_skeleton[:, c2]
                    ) / 2
            ...
            if i == 22 and not naive:
                ...
                child_final_loc_top = torch.matmul(
                    rot_mat_chain[parents[i]].transpose(1, 2), rel_pose_skeleton[:, 70]
                )

                _, sin1, cos1 = vectors2aa(child_final_loc_top, child_final_loc_bottom)
                ...
                rot2, axis2, _, _ = vectors2rotmat2(
                    child_rest_loc_top,
                    child_rest_loc_bottom,
                    dtype=child_final_loc_top.dtype,
                )
...
def batch_get_pelvis_orient(rel_pose_skeleton, rel_rest_pose, parents, children, dtype):
    ...
    spine_final_loc = rel_pose_skeleton[
        :, pelvis_child[-1]
    ].clone()  # TODO: may get worse performance if choose other child ?
    ...
    assert torch.sum(torch.isnan(rot_mat_spine)) == 0, (
        "rot_mat_spine",
        rot_mat_spine,
        spine_final_loc,
        rel_pose_skeleton,
...
def batch_get_children_orient_svd(
...
def batch_get_children_orient(
    ...
    spine_final_loc = torch.matmul(
        rot_mat_chain_parent.transpose(1, 2), spine_final_loc
    )
    ...
    center_final_loc = torch.matmul(
        rot_mat_chain_parent.transpose(1, 2), center_final_loc
    )
...
def get_twist(rot_mats, joints, parents):
    ...
    for i in range(1, rot_mats.shape[1]):
        ...
        if childs[i] < 0:
            angle_twist.append(
                torch.zeros(
                    (batch_size, 1), dtype=rot_mats.dtype, device=rot_mats.device
                )
            )
            continue
...
def rotmat_to_aa(rotmat):
    ...
    angle_zero_mask = (-1e-6 < (r11 + r22 + r33 - 1) / 2 + 1) & (
        (r11 + r22 + r33 - 1) / 2 + 1 < 1e-6
    )

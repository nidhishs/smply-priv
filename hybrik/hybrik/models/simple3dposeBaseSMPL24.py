...
class Simple3DPoseBaseSMPL24(nn.Module):
    ...
    def uvd_to_cam(
        ...
        uv_homo_jts = torch.cat(
            (uvd_jts_new[:, :, :2], torch.ones_like(uvd_jts_new)[:, :, 2:]), dim=2
        )
    ...
    def forward(
        ...
        if self.norm_type == "sigmoid":
            ...
            maxvals = torch.ones(
                (*out.shape[:2], 1), dtype=torch.float, device=out.device
            )
        ...
        pred_uvd_jts_24_flat = pred_uvd_jts_24.reshape(
        ...
        if flip_item is not None:
            assert flip_output
            pred_uvd_jts_24_orig, pred_phi_orig, pred_leaf_orig, pred_shape_orig = (
                flip_item
            )

        if flip_output:
            pred_uvd_jts_24 = self.flip_uvd_coord(
                pred_uvd_jts_24, flatten=False, shift=True
            )
        if flip_output and flip_item is not None:
            pred_uvd_jts_24 = (
                pred_uvd_jts_24 + pred_uvd_jts_24_orig.reshape(batch_size, 24, 3)
            ) / 2
        ...
        #  -0.5 ~ 0.5
        # Rotate back
        pred_xyz_jts_24 = self.uvd_to_cam(
            pred_uvd_jts_24[:, :24, :],
            trans_inv,
            intrinsic_param,
            joint_root,
        ...
        assert torch.sum(torch.isnan(pred_xyz_jts_24)) == 0, (
            "pred_xyz_jts_24",
            pred_xyz_jts_24,
        )

        pred_xyz_jts_24 = pred_xyz_jts_24 - pred_xyz_jts_24[
            :, self.root_idx_24, :
        ].unsqueeze(1)

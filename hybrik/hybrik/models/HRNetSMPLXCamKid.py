...
class HRNetSMPLXCamKid(nn.Module):
    ...
    def update_scale(
        ...
        Ax = torch.zeros(
            (bs, num_joints, 1), device=pred_uvd.device, dtype=pred_uvd.dtype
        )
        Ay = torch.zeros(
            (bs, num_joints, 1), device=pred_uvd.device, dtype=pred_uvd.dtype
        )
    ...
    def forward(self, x, flip_test=False, **kwargs):
        ...
        out_uv = out_uv.reshape(
            batch_size, self.num_joints, self.height_dim * self.width_dim
        )
        ...
        if flip_test:
            ...
            # flip heatmap
            flip_out_uv = flip_out_uv.reshape(
                batch_size, self.num_joints, self.height_dim, self.width_dim
            )
        ...
        heatmaps_uv = heatmaps_uv.reshape(
            (batch_size, self.num_joints, self.height_dim, self.width_dim)
        )
        ...
        if flip_test:
            ...
            flip_sigma = (
                self.decsigma(flip_x0).reshape(batch_size, self.num_joints, 1).sigmoid()
            )

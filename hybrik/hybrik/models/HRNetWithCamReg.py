...
class HRNetSMPLCamReg(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        bbox_3d_shape = (
            kwargs["BBOX_3D_SHAPE"] if "BBOX_3D_SHAPE" in kwargs else (2000, 2000, 2000)
        )
    ...
    def update_scale(
        self, pred_uvd, weight, init_scale, pred_shape, pred_phi, **kwargs
    ):
        ...
        output = self.smpl.hybrik(
            pose_skeleton=pred_xyz.type(self.smpl_dtype)
            * self.depth_factor,  # unit: meter
    ...
    def forward(self, x, flip_test=False, **kwargs):
        ...
        if flip_test:
            ...
            flip_out_sigma = (
                self.decsigma(flip_x0).sigmoid().reshape(batch_size, self.num_joints, 1)
            )
        ...
        if "bboxes" in kwargs.keys():
            ...
            pred_xy_jts_29_meter = (
                (pred_uvd_jts_29[:, :, :2] + bbox_center)
                * self.input_size
                / self.focal_length
            ) * (
                pred_xyz_jts_29[:, :, 2:] * self.depth_factor + camDepth
            )  # unit: m
